...
- **Single-platform** — Affects only one platform. Create or update specs for that platform using the naming convention defined in CLAUDE.md.
...
- **Platform-specific variant**: Tell the agent which base spec to reference and have it create a platform-specific variant file (following the naming convention in CLAUDE.md) that covers only platform-specific requirements and divergences. The variant must reference the base spec and not duplicate shared requirements.
