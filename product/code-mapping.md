...
- **Scoped placement** `FR-code-mapping-marker-scope`: A marker is placed on or immediately above the smallest enclosing named unit (function, method, or block) that implements the requirement, not at file-top level.
...
- [ ] **Marker at file-top rejected** `AC-code-mapping-marker-scope-enforced`: A marker placed at file-top level when the file contains one or more named units capable of scoping the claim is flagged by the audit and does not count toward coverage.
